                    ...
                    <div class="col-lg-12">
                            <?php
								if(isset($_POST['submit'])) {
									...
									if (($dsure == 'yes') && ($user != NULL)) {
                                        //delete all logs
		                                $query = "DELETE FROM `logs` WHERE `user`='".$user."'";
                                        $result = mysql_query($query);
                                        confirm_query($result);
                                        if ($result) {
	                                        $obs = $user;
			                                $SYS = get_device($user_settings['user_name']);
			                                insert_log($lang,$SYS,$user_settings['user_name'],'error','k173',$obs);
                                            echo "<div class=\"panel panel-primary\">";
                                            echo "<div class=\"panel-heading\">
                                                      ".get_lang($lang, 'k122')."
                                                  </div>";
										    echo "<div class=\"panel-body\"><div class=\"alert alert-success\">";
                                            echo get_lang($lang, 'k172')." ".$user;
                                            echo "</div></div></div>";
				                            $url = "logs.php?lang=".$lang;
				                            redir($url,'2');
										} else {
                                            ...
                                            echo "<button type=\"button\" class=\"btn btn-primary btn-lg\" onClick=\"document.location.href = 'logs.php?lang=".$lang."'\"><i class=\"fa fa-arrow-left\"></i>&nbsp;".get_lang($lang, 'k34')."</button>";
                                            echo "</div></div>";
										}
									} else {
									//error not deleted
