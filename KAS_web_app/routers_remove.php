                    ...
                    <div class="col-lg-12">
                            <?php
								if(isset($_POST['submit'])) {
									$drouter_id = mysql_prep($_POST['id']);
									$drouter_name = mysql_prep($_POST['router_name']);
									$dsure = mysql_prep($_POST['sure']);

									if (($drouter_id != NULL) && ($dsure == 'yes')) {
										//try to update
	                                    mysql_query("SET NAMES utf8");
		                                $query = "DELETE FROM `routers` WHERE `id`='".$drouter_id."' AND `router_name`='".$drouter_name."'";
                                        $result = mysql_query($query);
                                        confirm_query($result);
                                        if ($result) {
                                            ...
                                            echo "<button type=\"button\" class=\"btn btn-primary btn-lg\" onClick=\"document.location.href = 'routers.php?lang=".$lang."'\"><i class=\"fa fa-arrow-left\"></i>&nbsp;".get_lang($lang, 'k34')."</button>";
                                            echo "</div></div>";
										}
									} else {
