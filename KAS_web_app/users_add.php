                    ...
                    <div class="col-lg-12">
                        <div class="panel panel-primary">
                            <div class="panel-heading">
                                <?php echo get_lang($lang, 'k97'); ?>
                            </div>
							<form method="POST" action="users_insert.php?lang=<?=$lang?>" id="forms">
							<div class="panel-body">
                                <div class="form-group">
                                    <label><?php echo get_lang($lang, 'k05'); ?>:</label>
                                    <input class="form-control validate[required,minSize[4]],custom[onlyLetterNumber]" maxlength="32" type="text" name="user_name" value="" />
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <input class="form-control validate[required]" maxlength="32" type="text" name="first_name" value="" />
                                </div>
                                <div class="form-group">
                                    <label><?php echo get_lang($lang, 'k87'); ?>:</label>
                                    <input class="form-control validate[required]" maxlength="32" type="text" name="last_name" value="" />
                                </div>
                                <div class="form-group">
                                    <label><?php echo get_lang($lang, 'k91'); ?>:</label>
                                    <input class="form-control" maxlength="64" type="text" name="email" value="" />
                                </div>
                                <div class="form-group">
                                    <label><?php echo get_lang($lang, 'k92'); ?>:</label>
                                    <input class="form-control" maxlength="32" type="text" name="phone" value="" />
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <input class="form-control" maxlength="64" type="text" name="info" value="" />
                                </div>
                                <div class="form-group">
                                    <label><?php echo get_lang($lang, 'k120'); ?>:</label>
                                    <input class="form-control" maxlength="17" type="text" name="device" value="" />
                                </div>
                <?php
				if ($user_settings['level'] > 10) {
					//all routers/buildings
                    $query1 = "SELECT `router_name` FROM `routers` ORDER BY `router_name` ASC";
                    $result1 = mysql_query($query1);
                    confirm_query($result1);
                    if (mysql_num_rows($result1) != 0) {
						echo "<div class=\"form-group\">
                                  <label>".get_lang($lang, 'k43').":</label>
                                  <div id=\"checks\">";
						while($routers = mysql_fetch_array($result1)) {
							$routers_all[] = $routers['router_name'];
							echo "<div class=\"checkbox-icheck\">
                                      <label class=\"checkbox-inline\">
                                          <span class=\"input\">
										      <input type=\"checkbox\" value=\"".$routers['router_name']."\" name=\"buildings[]\" />&nbsp;".$routers['router_name']."</span>
                                      </label>
							      </div>";
					    }
						echo "</div></div>";
					}
					if (!empty($routers_all)) {
						foreach ($routers_all as $router_b) {
                            $query = "SELECT `inv` FROM `klimatiki` WHERE `router`='".$router_b."' ORDER BY `inv` ASC";
                            $result = mysql_query($query);
                            confirm_query($result);
                            if (mysql_num_rows($result) != 0) {
						        echo "<div class=\"form-group\">
                                      <label>".get_lang($lang, 'k10')." ".$router_b.":</label>
                                          <div id=\"checks\">";
						        while($klimas = mysql_fetch_array($result)) {
							        echo "<div class=\"checkbox-icheck\">
                                              <label class=\"checkbox-inline\">
                                                  <span class=\"input\">
										              <input type=\"checkbox\" value=\"".$klimas['inv']."\" name=\"klimas[]\" />&nbsp;".$klimas['inv']."</span>
                                              </label>
							              </div>";
					            }
						        echo "</div></div>";
				...
				?>
                            ...
                            <div class="panel-footer">
								...
								<button type="submit" name="submit" class="btn btn-warning btn-lg">
								    <i class="fa fa-plus"></i>&nbsp;<?php echo get_lang($lang, 'k97'); ?></button>
                            </div>
							</form>
                        </div>
                    </div>
