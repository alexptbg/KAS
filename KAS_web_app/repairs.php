    ...
    <body>
        <div id="wrapper">
            <nav class="navbar navbar-default navbar-static-top" role="navigation">
                ...
                <ul class="nav navbar-top-links navbar-right">
                    ...
                    <li>
                        <a href="repairs_add.php?lang=<?=$lang?>">
            ...
            <div id="page-wrapper">
                ...
                if ($user_settings['level'] > 20) {
				    ...
				    $b_active = "disabled=\"disabled\"";
			    }
				if ($user_settings['level'] > 10) {
                    $query = "SELECT * FROM `maintenance` ORDER BY `date`,`id` DESC";
                    $result = mysql_query($query);
                    confirm_query($result);
                    if (mysql_num_rows($result) != 0) {
						//echo header
                ...
                <div class=\"row\">
                    <div class=\"col-lg-12\">
                        <div class=\"panel panel-primary\">
                            <div class=\"panel-heading\">
                                ...
                                <div class=\"btn-group pull-right\">
                                    <button type=\"button\" class=\"btn btn-warning\" style=\"margin-top:-10px;margin-right:-16px;border-radius:0;padding-top:9px;padding-left:16px;padding-right:16px;padding-bottom:9px;\" onclick=\"location.href='repairs_add.php?lang=".$lang."';\">
                            ...
                            <div class=\"panel-body\">
								...
								<div class=\"table-responsive\">
                                    <table class=\"table table-striped table-bordered table-hover\" id=\"repairs\" style=\"margin-bottom:5px;\">
                                        <thead>
                                            <tr>
                                                <th>".get_lang($lang,'k117')."</th>
                                                <th>".get_lang($lang,'k14')."</th>
		                ...
		                while($repairs = mysql_fetch_array($result)) {
							...
							if (($repairs['user'] != 'Alex') && ($repairs['user'] != 'alex')) {
								...
								echo   "<td>".$repairs['router']."</td>
										...
										<td>
						                    <button type=\"button\" class=\"btn btn-warning btn-sm\" onClick=\"document.location.href='repairs_edit.php?lang=bg&kinv=".$repairs['inv']."&rid=".$repairs['id']."'\"><i class=\"fa fa-edit\"></i></button>
						                    <button type=\"button\" class=\"btn btn-danger btn-sm\" onClick=\"confirm_del('".$repairs['id']."','".$repairs['inv']."'); return false;\" ".$b_active."><i class=\"fa fa-trash-o\"></i></button>
										</td>
									  </tr>";
							}
						}
					    ...
					    echo "
                            ...
                            if ($user_settings['level'] > 20) {
                            	echo "
                              <button type=\"button\" class=\"btn btn-danger btn-lg\" onClick=\"document.location.href = 'repairs_del_all.php?lang=".$lang."'\">
							    <i class=\"fa fa-trash-o\"></i>&nbsp;".get_lang($lang,'k286')." <small>(".decodeSize(get_table_size('maintenance')).")</small></button>";
							} else {
                            	echo "
                              <button type=\"button\" class=\"btn btn-danger btn-lg\" onClick=\"document.location.href = 'repairs_del_all.php?lang=".$lang."'\" disabled=\"disabled\">
							    <i class=\"fa fa-trash-o\"></i>&nbsp;".get_lang($lang,'k286')." <small>(".decodeSize(get_table_size('maintenance')).")</small></button>";
							}
							echo "
                            </div>
        ...
        $(function () {
         ...
         if($("#repairs").length > 0) { 
             $('#repairs').dataTable({
			     "aaSorting": [[ 0, "desc" ]],
				 "iDisplayLength": 25,
				 "aLengthMenu": [[25, 50, 100, -1], [25, 50, 100, "Всички"]],
                 "aoColumns": [
				     { "bSearchable": false },
					 { "bSortable": false },
					 { "bSortable": false },
		...
		.columnFilter({
		    aoColumns:[
				...
				{ sSelector: "#router", type:"select" },
				{ sSelector: "#inv", type:"select" },
				{ sSelector: "#who", type:"select" },
				{ sSelector: "#user", type:"select" },
				null,
				null
			]
		});
        ...
        function confirm_del(rid,kinv) {
	        var answer = confirm("<?php echo get_lang($lang,'k290'); ?>");
	        if (answer){ repair_del(rid,kinv); }
	        else { alert("<?php echo get_lang($lang,'k291'); ?>"); }
        }
        function repair_del(rid,kinv) {
            $.ajax({
                type: "POST",
                url: "repairs_del.php?rid="+rid+"&kinv="+kinv,
                success: function(){
	                alert("<?php echo get_lang($lang,'k292'); ?>");
                    var delay = 1000;
                    setTimeout(function(){ window.location.href = 'repairs.php?lang=bg'; }, delay);
                }, 
                error: function(){
                	alert("<?php echo get_lang($lang,'k33'); ?>");
                }
            });
            return false;
        }
