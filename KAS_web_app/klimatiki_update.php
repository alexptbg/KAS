                    ...
                    <div class="col-lg-12">
                            <?php
								if(isset($_POST['submit'])) {
									...
									$uk_router = mysql_prep($_POST['k_router']);
									$uk_inv = mysql_prep($_POST['inv']);
									$uk_rtu = mysql_prep($_POST['rtu']);
									$uk_addr = mysql_prep($_POST['addr']);
									$uk_building = mysql_prep($_POST['building']);
									$uk_floor = mysql_prep($_POST['floor']);
									$uk_group = mysql_prep($_POST['group']);
									$uk_zona = mysql_prep($_POST['zona']);
									$uk_place = mysql_prep($_POST['place']);
									$uk_type = mysql_prep($_POST['type']);
									$uk_vrf = mysql_prep($_POST['vrf']);
									...
									$check = check_klimatik_id_and_inv_and_rtu($uk_id,$uk_inv,$uk_rtu);
									if (($uk_id != NULL) && ($check == FALSE)) {
										//try to update
	                                    mysql_query("SET NAMES utf8");
		                                $query = "UPDATE `klimatiki` SET `router`='$uk_router', `inv`='$uk_inv', `rtu`='$uk_rtu', `addr`='$uk_addr', `building`='$uk_building', `floor`='$uk_floor', `group`='$uk_group', `zona`='$uk_zona', `place`='$uk_place', `type`='$uk_type', `vrf`='$uk_vrf', `prog`='$uk_prog', `start_w`='$uk_start_w', `stop_w`='$uk_stop_w', `start_s`='$uk_start_s', `stop_s`='$uk_stop_s' WHERE `id`='$uk_id'";
                                        $result = mysql_query($query);
                                        confirm_query($result);
                                        if ($result) {
