                ...
                <div class="row">
                    <div class="col-lg-12">
                        <h3 class="page-header text-asbestos">
						    <i class="fa fa-square"></i> <?php echo get_lang($lang, 'k68'); ?> <?php echo " \ ".$b." \ ".$f." \ ".$i; ?></h3>
                    </div>
                </div>
                <!-- /.col-lg-12 -->
                <?php if ($user_settings['level'] > 2): ?>
                <div class="row">
                    <?php include('inc/tables.php'); ?>
                    <script type="text/javascript">
                        $(function() {
 	                        $('#data').load('k_i.php?lang=<?=$lang?>&r=<?=$r?>&b=<?=$b?>&f=<?=$f?>&i=<?=$i?>&x=');
                            var refreshId = setInterval(function() {
				                if(!ispaused){
                                    $('#data').load('k_i.php?lang=<?=$lang?>&r=<?=$r?>&b=<?=$b?>&f=<?=$f?>&i=<?=$i?>&x='+ Math.random()); }
                            }, 10000);
