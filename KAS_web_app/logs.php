...
<head>
    ...
    <body>
		                ...
		                while($logs = mysql_fetch_array($result)) {
							...
							if ($logs['user'] != 'Alex') {
								...
								if (date('Y-m-d') == date('Y-m-d', strtotime($logs['date']))) {
									echo "<td class=\"log-success\">".$logs['date']."</td>";
								} else {
									echo "<td>".$logs['date']."</td>";
								}		
								echo   "<td>".$logs['time']."</td>
										<td>".$logs['device']."</td>
										<td class=\"userx\"><a href=\"logs_user.php?lang=".$lang."&user=".$logs['user']."\">".$logs['user']."</a></td>
										<td class=\"log-".$logs['filter']."\">".get_lang($lang,$logs['action'])." (".$logs['obs'].")</td>
									</tr>";
							}
						}
						//echo footer
		...
		.columnFilter({
		    aoColumns:[
				null,
				{ sSelector: "#date", type:"input" },
				{ sSelector: "#time", type:"input" },
				{ sSelector: "#device", type:"select" },
				{ sSelector: "#user", type:"input" },
				null
			]
		});
        }
