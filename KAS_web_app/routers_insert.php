                    ...
                    <div class="col-lg-12">
                            <?php
								if(isset($_POST['submit'])) {
									$arouter_name = mysql_prep($_POST['router_name']);
									$arouter_sname = mysql_prep($_POST['router_sname']);
									$adata_file = mysql_prep($_POST['data_file']);
									$aip_address = mysql_prep($_POST['ip_address']);
									$aport = mysql_prep($_POST['port']);
									$aex_table = mysql_prep($_POST['ex_table']);
									$aplace = mysql_prep($_POST['place']);
									$check = check_router_name($arouter_name);
									if (($arouter_name != NULL) && ($check == FALSE)) {
										//try to update
	                                    mysql_query("SET NAMES utf8");
		                                $query = "INSERT INTO `routers` (`router_name`, `router_sname`, `data_file`, `ip_address`, `port`, `ex_table`, `place`) VALUES ('$arouter_name', '$arouter_sname', '$adata_file', '$aip_address', '$aport', '$aex_table', '$aplace')";
                                        $result = mysql_query($query);
                                        confirm_query($result);
                                        if ($result) {
