                    ...
                    <div class="col-lg-12">
                            <?php
								if(isset($_POST['submit'])) {
                                    $uk_date = mysql_prep($_POST['date']);
									$uk_router = mysql_prep($_POST['router']);
									$uk_inv = mysql_prep($_POST['invs']);
									$uk_who = mysql_prep($_POST['who']);
									$uk_action = mysql_prep($_POST['action']);
									$uk_obs = mysql_prep($_POST['obs']);
									if (($uk_router != NULL) && ($rid != NULL)) {
										//try to update
		                                $query = "UPDATE `maintenance` SET `date`='".$uk_date."', `router`='".$uk_router."', `inv`='".$uk_inv."', `who`='".$uk_who."', `user`='".$user_settings['user_name']."', `action`='".$uk_action."', `obs`='".$uk_obs."' WHERE `id`='".$rid."'";
                                        $result = mysql_query($query);
                                        confirm_query($result);
                                        if ($result) {
                                        	//inserted
                                            ...
                                            echo get_lang($lang,'k289');
                                            echo "</div></div></div>";
				                            $url = "repairs.php?lang=".$lang;
				                            redir($url,'2');
                                        } else {
											//error with insert
                                            echo "<div class=\"panel panel-primary\">";
