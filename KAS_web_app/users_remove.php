                    ...
                    <div class="col-lg-12">
                            <?php
								if(isset($_POST['submit'])) {
									$d_uid = mysql_prep($_POST['uid']);
									$d_user_name = mysql_prep($_POST['user_name']);
									$d_sure = mysql_prep($_POST['sure']);
									if (($d_uid != NULL) && ($d_sure == 'yes')) {
										//try to delete
	                                    mysql_query("SET NAMES utf8");
		                                $query = "DELETE FROM `users` WHERE `id`='".$d_uid."' AND `user_name`='".$d_user_name."'";
                                        $result = mysql_query($query);
                                        confirm_query($result);
                                        if ($result) {
