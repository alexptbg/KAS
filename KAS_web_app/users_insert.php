                    ...
                    <div class="col-lg-12">
                            <?php
								if(isset($_POST['submit'])) {
									$auser_name = mysql_prep($_POST['user_name']);
									$apassword = mysql_prep($_POST['password']);
									$apassword2 = mysql_prep($_POST['password2']);
									$afirst_name = mysql_prep($_POST['first_name']);
									$alast_name = mysql_prep($_POST['last_name']);
									$aemail = mysql_prep($_POST['email']);
									$aphone = mysql_prep($_POST['phone']);
									$access = mysql_prep($_POST['access']);
									$alevel = mysql_prep($_POST['level']);
									$astatus = mysql_prep($_POST['status']);
									$ailang = mysql_prep($_POST['init_lang']);
									$ainfo = mysql_prep($_POST['info']);
									$adevice = mysql_prep($_POST['device']);
									$abuildings = implode(", ",$_POST['buildings']);
									$aklimas = implode(", ",$_POST['klimas']);
									$check = check_user_name_before_add($auser_name);
									if (($auser_name != NULL) && ($check == FALSE)) {
										$passwd1 = md5($apassword); 
										$passwd2 = md5($apassword2);
										if ($passwd1 == $passwd2) {
										//try to insert
	                                    mysql_query("SET NAMES utf8");
		                                $query = "INSERT INTO `users` (`user_name`, `h_password`, `first_name`, `last_name`, `email`, `phone`, `access`, `level`, `status`, `init_lang`, `info`, `device`, `buildings`, `klimas`) VALUES ('".$auser_name."', '".$passwd2."', '".$afirst_name."', '".$alast_name."', '".$aemail."', '".$aphone."', '".$access."', '".$alevel."', '".$astatus."', '".$ailang."', '".$ainfo."', '".$adevice."', '".$abuildings."', '".$aklimas."')";
                                        $result = mysql_query($query);
                                        confirm_query($result);
                                        if ($result) {
									    ...
									    //error with double passwords
                                        echo "<div class=\"panel panel-primary\">";
                                        echo "<div class=\"panel-heading\">
                                                      ".get_lang($lang, 'k100')."
                                                  </div>";
										echo "<div class=\"panel-body\"><div class=\"alert alert-danger\">";
                                        echo get_lang($lang, 'k102');
                                        echo "</div></div>";
									    echo "<div class=\"panel-footer\">";
                                        echo "<button type=\"button\" class=\"btn btn-primary btn-lg\" onClick=\"javascript: history.go(-1); return false;\"><i class=\"fa fa-arrow-left\"></i>&nbsp;".get_lang($lang, 'k34')."</button>";
                                        echo "</div></div>";
